...
class PublicProductOptionDialog extends ConsumerStatefulWidget {
  final Product product;
  const PublicProductOptionDialog({Key? key, required this.product}) : super(key: key);

  @override
  ConsumerState<PublicProductOptionDialog> createState() => _PublicProductOptionDialogState();
}

class _PublicProductOptionDialogState extends ConsumerState<PublicProductOptionDialog> {
  int _qty = 1;
  ...
  Widget build(BuildContext context) {
    ...
    final sambalSettingsAsync = ref.watch(sambalSettingsProvider);
    ...
    return toppingsAsync.when(
      ...
      data: (allToppings) {
            ...
            children: [
              ...
              Container(
                ...
                child: const ListTile(
                  dense: true,
                  leading: Icon(Icons.lock, size: 16, color: Colors.grey),
                  title: Text('Pangsit (Default)', style: TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text('+ Rp 1.500 (Wajib)'),
              ...
              ...activeToppings.map((topping) {
                ...
                return Container(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text('Rp ${topping.price.toStringAsFixed(0)}'),
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        if (currentQty > 0)
        ...
        return AlertDialog(
          title: Text('Kustomisasi: ${widget.product.name}'),
          content: SizedBox(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                final currentSambalSettings = sambalSettingsAsync.value ?? const SambalSettings();

                ref.read(publicCartProvider.notifier).addItem(
                  widget.product,
                  _qty,
                  ...
                  settings: currentSambalSettings,
                );
